...
export const CookieConsentDialog = () => {
  ...
  const handleAcceptAll = () => {
    const allConsent = { necessary: true, analytics: true, thirdParty: true };
  ...
  const handleRejectAll = () => {
    const rejectAllConsent = { necessary: false, analytics: false, thirdParty: false };
    localStorage.setItem('cookieConsent', JSON.stringify(rejectAllConsent));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="w-11/12 max-w-lg rounded-lg bg-gray-800 p-6 text-white">
        <Typography variant="Bim1" className="mb-4 text-center text-xl">
          {dict.cookies.title}
        </Typography>
        <Typography variant="Bim4Regular" className="mb-6 text-sm">
          {dict.cookies.description}
        </Typography>
        <div className="mb-4 space-y-2">
          <div className="flex items-center justify-between">
            <Typography variant="Bim4Regular" className="text-sm">
              {dict.cookies.necessary}
            </Typography>
            <input type="checkbox"
              checked={consentChoices.necessary}
              onChange={() => handleToggleConsent('necessary')}
            />
          </div>
          <div className="flex items-center justify-between">
            <Typography variant="Bim4Regular" className="text-sm">
